...
while true do
	...
	local platerToPicture = pictureCenter - playerCFrame.Position
	local dot = playerCFrame.LookVector:Dot(platerToPicture.Unit)
 
	if dot > ((platerToPicture.Magnitude <= 20) and .3 or .85) then
		headDummy.Neck.C0 = headDummy.Neck.C0:Lerp(neckC0, deltaTime * headSpeed)
 
	else
		local direction = pictureCenter - character:GetPivot().Position
 
		local cframeTraget = (Matrix.byAngleX(math.clamp((math.atan2(direction.Y, direction.X) + math.pi) % tau, minAngleX, maxAngleX))
			* Matrix.byAngleY(math.atan2(direction.Z, direction.X) + math.pi)):GetCFrame(neckC0Position)
